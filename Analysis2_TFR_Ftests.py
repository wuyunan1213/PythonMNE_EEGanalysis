...
####do the F-test for channel FZ
words = ['TAR', 'CON']

power_dict = dict()
itc_dict = dict()
decim = 2
freqs = np.arange(5, 54, 3)  # define frequencies of interest
n_freqs = len(freqs)
...
epochs_power = epochs_power.reshape(n_replications, n_conditions, n_freqs*n_times)
epochs_itc = epochs_itc.reshape(n_replications, n_conditions, n_freqs*n_times)
...
for effect, sig, effect_label in zip(fvals, pvals, effect_labels):
    ...
    plt.imshow(effect.reshape(len(freqs), 601), cmap=plt.cm.gray, extent=[times[0],
               times[-1], freqs[0], freqs[-1]], aspect='auto',
               origin='lower')
    # create mask for significant Time-frequency locations
    effect = np.ma.masked_array(effect, [sig > .05])
    plt.imshow(effect.reshape(len(freqs), 601), cmap='RdBu_r', extent=[times[0],
               times[-1], freqs[0], freqs[-1]], aspect='auto',
               origin='lower')
    plt.colorbar()
    plt.xlabel('Time (ms)')
...
fvals, pvals = f_mway_rm(epochs_itc, factor_levels, effects=effects)
effect_labels = ['word', 'session', 'word by session']
...
#######looking at cluster-level significance
    ###for itc
epochs_itc = np.swapaxes(np.asarray(epochs_itc), 1, 0)
effects = 'A:B'
def stat_fun(*args):
    return f_mway_rm(np.swapaxes(args, 1, 0), factor_levels=factor_levels,
                     effects=effects, return_pvals=False)[0]
